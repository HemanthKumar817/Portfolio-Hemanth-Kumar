...
import { LiquidEtherProps } from '../types';

// Helper for encoding/decoding base64 if needed, but here we use native Three.js helpers
// Pre-defining shaders as constants to avoid re-creation
const face_vert = `
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class AutoDriver {
      ...
      constructor(private mouse: MouseClass, private manager: any, opts: any) {
    ...
    class ShaderPass {
      ...
      // Fixed: accept optional arguments to be compatible with subclasses
      init(args?: any): void {
      ...
      // Fixed: accept optional arguments and return any to be compatible with subclasses
      update(args?: any): any {
    ...
    // Expose update function to avoid re-initialization
    webgl.updateParams = (props: any) => {
      ...
      if (props.colors && webgl.output) {
        const newPalette = makePaletteTexture(props.colors);
        webgl.output.output.material.uniforms.palette.value = newPalette;
      }
    };
  ...
  // Efficiently update params when they change without re-initializing WebGL
  useEffect(() => {
    if (webglRef.current && webglRef.current.updateParams) {
      webglRef.current.updateParams({
        mouseForce, cursorSize, isViscous, viscous, iterationsViscous, iterationsPoisson, dt, BFECC, resolution, isBounce, colors
      });
    }
  }, [mouseForce, cursorSize, isViscous, viscous, iterationsViscous, iterationsPoisson, dt, BFECC, resolution, isBounce, colors]);
